...
7. 커밋과 Git 제외 파일의 별도 보존을 모두 확인한 뒤, 사용자에게 정리 범위를 알리고 동의받은 worktree와 브랜치만 정리한다. 병합 완료만으로 폴더를 자동 삭제하지 않는다.
...
- `git status`가 clean이어도 `.env.local`·`.dev.vars`·`.superpowers/sdd/` 등 ignored 파일은 남아 있을 수 있다. `git status --short --ignored`로 파일 목록을 확인하되 비밀값은 출력하지 않는다.
- `git worktree remove`도 ignored 파일을 함께 삭제할 수 있다. `--force`를 쓰지 않는다는 것만으로 비밀 설정이 보호되지 않는다.
- 환경변수 원본은 worktree 밖의 사용자 승인 경로 또는 password manager에 보관한다. 로컬 평문 사본이 필요하면 소유자만 읽는 권한과 디렉터리 접근 제한을 적용하고, PC 이전용 사본은 암호화한다. 경로는 사용자와 확인하고 임의로 비밀 파일을 복제하지 않는다.
- 삭제 전 필요한 파일이 별도 위치에 실제 존재하는지 확인한다. 백업 완료 여부를 확인할 수 없으면 worktree 정리를 중단하고 그대로 보존한다. 비밀값이나 해시는 문서·대화에 남기지 않는다.
