...
**Spec:** `docs/superpowers/specs/2026-09-01_공개용_24시간_추정가_설계.md`
...
```bash
rg -n "TOSS_INVEST|Toss Invest|fetchToss|provider: 'toss'|토스증권" app components hooks lib tests README.md docs/product/프로젝트_소개.md .env.example
```
...
`프로젝트_소개.md`의 제품 정의를 `실시간 주식 대시보드`에서 `24시간 시장 참고 대시보드`로 고치고 실제값·추정가 혼동 금지 원칙을 유지한다.
...
```bash
rg -n "TOSS_INVEST|fetchToss|provider: 'toss'" app components hooks lib tests README.md docs/product/프로젝트_소개.md
```
...
```bash
git add README.md docs/product/프로젝트_소개.md CHANGELOG.md package.json lib/market/providers/toss.ts lib/toss-invest.ts lib/market/previous-close.ts lib/market/session.ts tests/market/toss-provider.test.ts tests/market/previous-close.test.ts tests/market/session.test.ts .env.example
