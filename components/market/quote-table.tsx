...
export function QuoteTable({ quotes, nameLocale = 'ko' }: { quotes: MarketQuote[]; nameLocale?: NameLocale }) {
  ...
  return <div className="overflow-hidden rounded-2xl border bg-card/75 shadow-2xl shadow-black/10 backdrop-blur">
    ...
    {quotes.map((quote) => <article key={`${quote.assetClass}-${quote.symbol}`} className="grid grid-cols-[1fr_auto] items-center gap-3 border-b px-4 py-4 transition-colors last:border-0 hover:bg-muted/25 md:grid-cols-[1.4fr_1fr_.8fr_1fr_1.2fr] md:px-5">
      <div className="min-w-0"><h3 className="truncate font-bold">{nameLocale === 'en' ? quote.nameEn ?? quote.name : quote.nameKo ?? quote.name}</h3><p className="mt-0.5 font-mono text-xs text-muted-foreground">{quote.symbol}</p></div>
