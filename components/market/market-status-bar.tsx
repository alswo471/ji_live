...
export function MarketStatusBar({ state, fetchedAt }: { state: 'loading' | 'ready' | 'degraded' | 'stale' | 'error'; fetchedAt?: string }) {
  const label = state === 'ready' ? '시장 데이터 연결됨' : state === 'degraded' ? '일부 시장 데이터 지연·미제공' : state === 'stale' ? '마지막 정상 데이터 표시 중' : state === 'error' ? '시장 데이터 연결 재시도 중' : '시장 데이터 불러오는 중';
