...
export interface CommunityAdminSummaryDependencies {
  enabled: () => boolean;
  requireAdmin: (request: Request) => Promise<CommunityAdmin>;
  loadSummary: () => Promise<unknown>;
}

const dependencies: CommunityAdminSummaryDependencies = {
...
export async function handleAdminSummaryRequest(
  request: Request,
  deps: CommunityAdminSummaryDependencies = dependencies,
) {
  if (!deps.enabled()) {
  ...
  try {
    ...
    return json(await deps.loadSummary());
  } catch (error) {
...
export async function GET(request: Request) {
  return handleAdminSummaryRequest(request);
}
