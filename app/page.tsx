  ...
  return <main className="min-h-screen bg-background text-foreground">
    ...
    <div className="relative mx-auto min-h-screen w-full max-w-[1440px] border-x border-border bg-background/80">
      ...
      <div className="px-4 pb-12 pt-8 sm:px-6 lg:px-8">
        <section><div className="mb-5"><p className="flex items-center gap-2 text-xs font-bold uppercase tracking-[.16em] text-primary"><Activity className="size-4" />Live market pulse</p><h2 className="mt-2 text-3xl font-black tracking-[-.055em] sm:text-5xl">시장의 지금을 한 화면에.</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground sm:text-base">한국·미국 주식과 암호화폐의 실제 시세를 세션 상태와 함께 확인합니다. 장 마감 후 참고 추정가는 실제 시세와 명확히 구분합니다.</p></div><IndicatorGrid quotes={indicators} nameLocale={preferences.nameLocale} /></section>
        ...
        <footer className="mt-10 border-t border-border py-6 text-xs leading-5 text-muted-foreground">표시 정보는 투자 참고용이며 투자 판단의 근거로 사용할 수 없습니다. QQQ와 PAXG는 각각 해당 상품의 실제 가격이며 지수·현물 금 가격이 아닙니다.</footer>
      </div>
    </div>
  </main>;
}
