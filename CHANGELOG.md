...
- Toss·Binance·Bithumb 캔들을 공통 OHLCV 형식으로 제공하는 종목별 차트 API 추가
- 주식·암호화폐 종목별 1일·1주·1개월 가격·거래량 상세 차트 추가
- 상세 차트에 1분·15분·1시간·4시간·일봉·주봉·월봉 7개 주기와 주기별 기본 표시 범위 추가
- KOSPI 현재 지수와 Toss 전일 일봉을 이용한 실제 등락률 계산 추가
...
- USD/KRW는 임의 퍼센트 대신 Toss가 제공하는 상승·하락·보합 방향으로 표시
...
- KOSPI 등락률이 실제 계산 없이 `집계 중`으로 표시되던 문제 수정
- candle 빈 응답의 장기 cache와 주기 전환·polling 중 이전 응답이 최신 결과를 덮어쓰던 문제 수정
- Toss candle 전체 요청에 10초 제한을 강제하고 불완전한 조회 결과를 정상 cache하지 않도록 수정
