...
- 제공 가능한 종목의 1분·15분·1시간·4시간·일봉·주봉·월봉 OHLC·거래량 상세 차트
...
- **Binance Public API**: 검증된 한국·미국 주식 연동 무기한선물과 PAXG 현물 가격
- **Bithumb Public API**: 원화 암호화폐 가격과 `KRW-USDT` 합성환율

한국·미국 주식 가격은 KRX 또는 미국 거래소의 실제 체결가가 아니라 해외 파생상품 기반 24시간 추정가입니다. 등락률은 공급자가 제공한 `24시간 전 대비`를 사용합니다. 한국 주식은 파생상품 가격에 Bithumb `KRW-USDT` 가격을 곱해 원화로 환산하며, 이 값은 은행 고시환율이 아닌 합성환율입니다.

암호화폐와 PAXG는 표시된 거래상품의 실제 가격입니다. 공급자, 가격 성격과 마지막 기준 시각을 함께 확인해야 하며, 데이터가 없거나 오래되면 임의 가격을 만들지 않고 상태를 표시합니다.

미국 주식 파생상품, 암호화폐와 PAXG는 공급자 candle이 있는 경우 상세 차트를 제공합니다. 한국 주식 원화 환산 상세 차트는 과거 시점별 합성환율이 필요하므로 `0.4.0`에서 지원하지 않습니다.
