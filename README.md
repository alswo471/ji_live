...
- 시장별 상단 메뉴와 한글·영문명·심볼 검색, 시장 요약 중심 화면 ([UI 개편 1차 기록](./docs/product/2026-09-07_UI_개편_1차.md))
- 승인된 B2 캐릭터 로고와 카카오톡 피드백·광고 문의 연결
- Alternative.me 비트코인 공포·탐욕 일별 지수와 출처·기준일 표시
- ECB 기준 USD·100 JPY·THB 원화 교차환율, 발표일과 직전 발표일 대비 등락률 표시 (실시간 환율 아님)
- 홈 로고 위 금·환율 시세 전광판(별도 버튼·스크롤바 없이 hover·초점 정지 및 움직임 줄이기 지원), `/indicators` 시장 지표 페이지와 하단 출처·산정 기준 팝업. 비트코인 공포·탐욕 그래프는 BTC 상세·시장 지표 두 곳에 표시. USDT/KRW는 내부 환산용으로 유지
- 국내 ETF 일별 거래량 TOP 10과 코스피·코스닥 공식 일별 기록. 뉴스·나스닥 및 추가 종목 확장은 후속 작업
...
- 댓글 기본 이모지 선택·커서 위치 삽입, 서버와 같은 1,000자 검증과 중복 제출 방지.
- 한 단계 대댓글 구현: 원댓글 삭제·숨김 시 원문과 작성자 대신 안내를 표시하고 공개 답글은 유지합니다. 현재 원격 DB는 후속 migration 적용 전이므로 미리보기에는 답글 버튼을 노출하지 않습니다. [구현·적용 경계](./docs/product/2026-09-09_대댓글_구현이력.md)
- 커뮤니티 전체글·공지사항·인기글 메뉴(PC 좌측 / 모바일 상단), 전체 기간 추천순 인기글과 새로고침. 최소 추천 수 제한 없이 0개 글도 포함하며 동점은 최신순입니다. [구현·판단 이력](./docs/product/2026-09-09_커뮤니티_메뉴와_인기글.md)
...
종목 상세는 가격 다음에 차트를 보여주며 종목별 공급자·산식은 ‘이 종목의 데이터 근거’ 팝업에서 확인할 수 있습니다. 국내 주식 상세에는 최근 90일 내 제공된 공식 일별 기록을 별도 표시합니다. 코스피·코스닥 및 국내 ETF 거래량 TOP 10 역시 일별 기록이며 실시간 시세가 아닙니다. [구현 범위·공급자·캐시 제약](./docs/product/2026-09-07_공식_일별데이터_연동.md)을 참고하세요.

공공데이터포털의 금융위원회 주식·지수·증권상품 API를 각각 활용신청한 후 서버 `.env.local`에 `DATA_GO_KR_SERVICE_KEY`를 설정합니다. Encoding/Decoding 형식을 모두 지원하며 키를 클라이언트 환경변수나 Git에 넣지 않습니다. 키 미설정 시 준비 상태만 표시합니다. 원천 API는 사용자별 5초 호출이 아닌 프로세스별 1시간 캐시·동시 요청 병합을 사용합니다.
