...
- 종목 행에서 진입하는 1분·15분·1시간·4시간·일봉·주봉·월봉 OHLC·거래량 상세 차트
...
등락률은 공급자가 제공하면 그 값을 사용하고, 거래대금 상위 100위 밖 종목처럼 값이 없으면 `현재가 ÷ 전일 종가 - 1`로 계산합니다. KOSPI도 Toss 일봉의 전일 종가로 실제 등락률을 계산합니다. USD/KRW는 Toss가 전일 등락폭을 제공하지 않으므로 퍼센트를 만들지 않고 공급자가 제공한 상승·하락·보합 방향만 표시합니다. 상세 화면의 전일 종가가 비어 있으면 현재가와 공급자 등락률로 역산합니다. QQQ는 NASDAQ 지수가 아닌 실제 ETF, PAXG는 금 현물가가 아닌 실제 토큰 시세입니다.

Toss가 제공하는 원본 봉은 1분봉과 일봉입니다. 15분·1시간·4시간봉은 1분봉을, 주봉·월봉은 일봉을 서버에서 OHLCV로 합산합니다. 각 봉은 최근 2시간, 1일, 5일, 1개월, 6개월, 2년, 10년 범위를 기본으로 보여줍니다.

4시간봉은 최초 조회 시 여러 페이지의 1분봉이 필요하므로 요청당 최대 40페이지·10초 제한을 적용합니다. 제한 안에 전체 범위를 수집하지 못하면 불완전한 결과를 cache하지 않고 일시적인 연결 실패로 처리합니다. Oracle 공개 배포 전에는 여러 서버가 같은 결과를 재수집하지 않도록 공유 cache 또는 사전 집계를 추가해야 합니다.
...
- Oracle Cloud 공개 배포와 고정 출구 IP 구성
